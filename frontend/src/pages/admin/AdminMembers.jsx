...
function StatusDot({ status }) {
  return <span className={`inline-block w-2 h-2 rounded-full mr-2 flex-shrink-0 ${status === 'active' ? 'bg-[#1D9E75]' : 'bg-gray-300'}`} />
}

function AddMemberModal({ onClose, onSuccess }) {
  const [form, setForm] = useState({ full_name: '', email: '', role: 'student', phone: '', child_first_name: '', child_age: '' })
  ...
  async function handleSubmit() {
    ...
    try { await api.post('/admin/members', { ...form, child_age: form.child_age ? parseInt(form.child_age) : null }); onSuccess() }
    catch (e) { setError(e.message || 'Failed to create member') } finally { setLoading(false) }
  }
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <input className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#1D9E75]" value={form.full_name} onChange={e => setForm(f => ({ ...f, full_name: e.target.value }))} placeholder="Jane Smith" />
          </div>
          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Email</label>
            <input type="email" className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#1D9E75]" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} placeholder="jane@example.com" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Role</label>
              <select className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#1D9E75]" value={form.role} onChange={e => setForm(f => ({ ...f, role: e.target.value }))}>
                {ROLES.map(r => <option key={r} value={r}>{r.charAt(0).toUpperCase() + r.slice(1)}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Phone</label>
              <input className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#1D9E75]" value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))} placeholder="(626) 555-0100" />
            </div>
          </div>
          {form.role === 'parent' && (
            <div className="bg-[#E1F5EE] rounded-lg p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#1D9E75]" value={form.child_first_name} onChange={e => setForm(f => ({ ...f, child_first_name: e.target.value }))} placeholder="Alex" />
                </div>
                <div>
                  <label className="block text-xs text-gray-500 mb-1">Age</label>
                  <input type="number" min="3" max="18" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#1D9E75]" value={form.child_age} onChange={e => setForm(f => ({ ...f, child_age: e.target.value }))} placeholder="8" />
                </div>
              </div>
            </div>
          )}
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex justify-end gap-3">
          ...
          <button onClick={handleSubmit} disabled={loading} className="px-5 py-2 bg-[#064029] text-white text-sm font-semibold rounded-lg hover:bg-[#085041] disabled:opacity-50 transition-colors">{loading ? 'Creating…' : 'Create Member'}</button>
        </div>
      </div>
    </div>
  )
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm">
        <div className="px-6 py-5">
          ...
          <p className="text-sm text-gray-600 leading-relaxed">This will permanently delete their Clerk account and all associated data. This action cannot be undone.</p>
        </div>
        <div className="px-6 py-4 border-t border-gray-100 flex justify-end gap-3">
          <button onClick={onClose} className="px-4 py-2 text-sm font-medium text-gray-600 hover:text-gray-800">Cancel</button>
          <button onClick={onConfirm} disabled={loading} className="px-5 py-2 bg-red-600 text-white text-sm font-semibold rounded-lg hover:bg-red-700 disabled:opacity-50 transition-colors">{loading ? 'Deleting…' : 'Delete Account'}</button>
        </div>
      </div>
    </div>
  )
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <button onClick={copy} className="w-full py-2.5 bg-[#064029] text-white text-sm font-semibold rounded-lg hover:bg-[#085041] transition-colors">{copied ? '✓ Copied!' : 'Copy Link'}</button>
        </div>
      </div>
    </div>
  )
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm">
        ...
        <div className="px-6 py-4 space-y-4">
          ...
          {allInstructors.length === 0 ? <p className="text-sm text-gray-500 py-4 text-center">No active instructors found.</p> : (
            <>
              <TypeaheadSelect options={allInstructors.map(i => ({ value: i.id, label: i.full_name, sublabel: i.email }))}
                value={[...assignedIds][0] || ''}
                onChange={v => { const c = [...assignedIds][0]; if (c && c !== v) unassign(c); if (v && v !== c) assign(v) }}
                ...
                <div className="space-y-1">
                  {allInstructors.filter(i => assignedIds.has(i.id)).map(i => (
                    <div key={i.id} className="flex items-center justify-between bg-[#E1F5EE] rounded-lg px-3 py-2">
                      <div><p className="text-sm font-medium text-[#064029]">{i.full_name}</p><p className="text-xs text-gray-500">{i.email}</p></div>
                      <button onClick={() => unassign(i.id)} disabled={loading} className="text-xs font-semibold text-red-500 hover:text-red-700 disabled:opacity-50">Remove</button>
                    </div>
                  ))}
                </div>
              )}
      ...
      <div className="flex items-start justify-between px-6 py-5 border-b border-gray-100">
        <div>
          ...
          <div className="mt-2 flex items-center gap-2">
            ...
            {isPending && <span className="text-xs text-amber-600 bg-amber-50 px-2 py-0.5 rounded font-medium">Invite Pending</span>}
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            <button onClick={() => setEditing(!editing)} className="px-4 py-2 text-sm font-medium bg-[#E1F5EE] text-[#064029] rounded-lg hover:bg-[#1D9E75] hover:text-white transition-colors">{editing ? 'Cancel Edit' : 'Edit Profile'}</button>
            <button onClick={handleResetPassword} disabled={resetting || isPending} className="px-4 py-2 text-sm font-medium bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 disabled:opacity-40 transition-colors">{resetting ? 'Sending…' : 'Reset Password'}</button>
            <button onClick={() => setShowDelete(true)} className="px-4 py-2 text-sm font-medium bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition-colors">Delete Account</button>
          </div>
        ...
        {editing && (
          <div className="bg-gray-50 rounded-xl p-4 space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Full Name</label>
                <input className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#1D9E75]" value={form.full_name} onChange={e => setForm(f => ({ ...f, full_name: e.target.value }))} />
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Phone</label>
                <input className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#1D9E75]" value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))} />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Role</label>
                <select className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#1D9E75]" value={form.role} onChange={e => setForm(f => ({ ...f, role: e.target.value }))}>
                  {ROLES.map(r => <option key={r} value={r}>{r.charAt(0).toUpperCase() + r.slice(1)}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Status</label>
                <select className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#1D9E75]" value={form.status} onChange={e => setForm(f => ({ ...f, status: e.target.value }))}>
            ...
            <div className="flex justify-end">
              <button onClick={handleSave} disabled={saving} className="px-5 py-2 bg-[#064029] text-white text-sm font-semibold rounded-lg hover:bg-[#085041] disabled:opacity-50 transition-colors">{saving ? 'Saving…' : 'Save Changes'}</button>
            </div>
          </div>
        )}
...
export default function AdminMembers() {
  ...
  return (
    <AdminLayout>
      {showAddModal && <AddMemberModal onClose={() => setShowAddModal(false)} onSuccess={() => { setShowAddModal(false); fetchMembers() }} />}

      <div className="px-6 lg:px-10 py-6 flex flex-col h-full">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden flex-1 flex min-h-0">
          <div className="flex w-full h-full">

            {/* Left — list panel */}
            <div className={`flex flex-col border-r border-gray-100 ${selectedMember ? 'w-80 min-w-[280px] hidden md:flex' : 'flex-1'}`}>
                ...
                <div className="flex gap-2 flex-wrap">
                  {['all', ...ROLES].map(r => (
                    <button key={r} onClick={() => setRoleFilter(r)}
                      className={`px-3 py-1 text-xs font-semibold rounded-full transition-colors capitalize ${roleFilter === r ? 'bg-[#064029] text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
                ...
                <div className="flex gap-2">
                  {['all', 'active', 'inactive'].map(s => (
                    <button key={s} onClick={() => setStatusFilter(s)}
                      className={`px-3 py-1 text-xs font-semibold rounded-full transition-colors capitalize ${statusFilter === s ? 'bg-[#1D9E75] text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
              ...
              <div className="flex-1 overflow-y-auto">
                {loading ? (
                  ...
                  <button key={m.id} onClick={() => navigate(`/admin/members/${m.id}`)}
                    className={`w-full text-left px-5 py-4 border-b border-gray-50 hover:bg-gray-50 transition-colors ${selectedId === m.id ? 'bg-[#E1F5EE] border-l-4 border-l-[#1D9E75]' : ''}`}>
              ...
              <div className="px-5 py-3 border-t border-gray-100 text-xs text-gray-400 flex-shrink-0">
            ...
            {selectedMember ? (
              <div className="flex-1 overflow-hidden">
                <MemberDetail member={selectedMember} onClose={() => navigate('/admin/members')} onRefresh={fetchMembers} allInstructors={allInstructors} />
              </div>
            ) : (
